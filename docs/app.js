...
function renderOverview() {
  ...
  $("#health-bars").innerHTML = `<div><span>Healthy Â· 25%+</span><strong>${number.format(healthy)}</strong></div><div class="bar"><i style="width:${rate}%"></i></div><div><span>Needs review</span><strong>${number.format(items.length - healthy)}</strong></div><div class="bar review"><i style="width:${100 - rate}%"></i></div>`;
  ...
  $("#pulse-link").innerHTML = recent.length ? "View all <span>â†’</span>" : "Review items <span>â†’</span>";
  $("#pulse-link").onclick = () => setView(recent.length ? "history" : "catalog");
  $("#pulse-list").innerHTML = (recent.length ? recent.map((entry) => `<button class="pulse-row" data-item="${escapeHtml(entry.itemId)}"><span class="pulse-icon">â†—</span><span><strong>${escapeHtml(entry.itemId)}</strong><small>${escapeHtml(entry.reason)}</small></span><b>${money(entry.to)}</b></button>`).join("") : watchlist.map((item) => `<button class="pulse-row" data-item="${escapeHtml(item.id)}"><span class="pulse-icon warning">!</span><span><strong>${escapeHtml(item.id)}</strong><small>${margin(item).toFixed(1)}% margin Â· review recommended</small></span><b>${money(item.price)}</b></button>`).join(""));
...
function renderDetail() {
  ...
  $("#detail-card").innerHTML = `<div class="detail-top"><div><div class="section-kicker">Item detail</div><h3>${escapeHtml(item.id)}</h3><p>SKU ${String(item.index).padStart(4, "0")} Â· ${itemHistory.length ? "Has price history" : "Workbook baseline"}</p></div><span class="item-avatar large">${initials(item.id)}</span></div><div class="price-hero"><span>Current price</span><strong>${money(item.price)}</strong><button data-detail-edit="${escapeHtml(item.id)}">Edit price <span>â†’</span></button></div><div class="detail-stats"><div><span>Unit cost</span><strong>${money(item.cost)}</strong></div><div><span>Gross margin</span><strong class="${margin(item) < 25 ? "warn-text" : "good-text"}">${margin(item).toFixed(1)}%</strong></div><div><span>Markup</span><strong>${item.cost ? ((item.price / item.cost - 1) * 100).toFixed(1) : "0.0"}%</strong></div></div><div class="history-head"><div><div class="section-kicker">Audit trail</div><h4>Price history</h4></div><span class="history-count">${itemHistory.length}</span></div><div class="history-list">${itemHistory.length ? itemHistory.map((entry) => `<div class="history-entry"><span class="timeline-line"></span><div class="history-dot"></div><div class="history-copy"><strong>${money(entry.from)} <span>â†’</span> ${money(entry.to)}</strong><p>${escapeHtml(entry.reason)}</p><small>${new Date(entry.changedAt).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })} Â· ${escapeHtml(entry.changedBy || "You")}</small></div></div>`).join("") : '<div class="history-empty"><span>âŒ</span><p>Price changes for this item will appear here with the reason and timestamp.</p></div>'}</div>`;
...
function renderHistory() {
  ...
  $("#history-content").innerHTML = sorted.length ? `<div class="history-table">${sorted.map((entry) => `<button class="history-table-row" data-history-item="${escapeHtml(entry.itemId)}"><span class="history-table-icon">â†—</span><span class="history-table-item"><strong>${escapeHtml(entry.itemId)}</strong><small>${new Date(entry.changedAt).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })} Â· ${escapeHtml(entry.changedBy || "You")}</small></span><span class="history-table-reason">${escapeHtml(entry.reason)}</span><span class="history-table-price"><strong>${money(entry.from)} <i>â†’</i> ${money(entry.to)}</strong><small>price update</small></span></button>`).join("")}</div>` : '<div class="empty-history"><span>âŒ</span><h3>No changes yet</h3><p>Once you update an item price, its full audit trail will appear here.</p><button class="primary-button" data-view="catalog">Browse catalog <span>â†’</span></button></div>';
...
function openEdit(id) {
  ...
  editingId = id; $("#edit-title").textContent = id; $("#edit-subtitle").innerHTML = `Current price <strong>${money(item.price)}</strong> Â· Cost ${money(item.cost)}`; $("#new-price").value = item.price.toFixed(2); $("#change-reason").value = ""; $("#edit-modal").hidden = false; $("#new-price").focus();
}
